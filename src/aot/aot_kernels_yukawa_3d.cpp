...
residual_evaluator_func<Real> get_yukawa_3d_kernel(dmk_eval_type eval_level, int n_digits,
    ...
    if (eval_level == DMK_POTENTIAL) {
        if (n == 3) {
            ...
            return [=](Real rsc, Real cen, Real d2max, Real thresh2, int n_src, const Real *r_src, const Real *charge,
                ...
                yukawa_3d_poly_all_pairs<Real, MaxVecLen, -1, NC0, DMK_POTENTIAL>(
        ...
        if (n == 3) {
            ...
            return [=](Real rsc, Real cen, Real d2max, Real thresh2, int n_src, const Real *r_src, const Real *charge,
                       const Real *normals, int n_trg, const Real *r_trg, Real *pot) {
                yukawa_3d_poly_all_pairs<Real, MaxVecLen, -1, NC0, DMK_POTENTIAL_GRAD>(
                    eval_level, n_digits, rsc, cen, d2max, thresh2, NC0, cf.data(), n_src, r_src, charge, normals,
                    n_trg, r_trg, pot, UF);
            };
        }
...
get_yukawa_3d_kernel<float, sctl::DefaultVecLen<float>()>(dmk_eval_type, int, const std::vector<std::vector<float>> &);
template residual_evaluator_func<double>
get_yukawa_3d_kernel<double, sctl::DefaultVecLen<double>()>(dmk_eval_type, int,
