...
c
c
      subroutine treedata_eval_pg_nd_asym_fast(ndim,nd,ndelta,deltas,
     1    dwhts,idelta,ipoly,nasym,nlevels,itree,iptr,boxsize,norder,
     2    fvals,flvals,fl2vals,gvals,glvals,pot,grad)
c
c     This code evaluates the potential value at the tensor grid on an adaptive tree
c     using asympotitic expansions with precompted Laplacian and biLaplacian
c 
...
c           BiLaplacian of the function
c
c     gvals - double (nd,ndim,norder**ndim,nboxes)
c           gradient of function values on the tree
c     glvals - double (nd,ndim,norder**ndim,nboxes)
c           Laplacian of gradient of the function
c
c     output:
c     pot - double precision (nd,norder**ndim,nboxes)
c           potential values on the tree
c     grad - double precision (nd,ndim,norder**ndim,nboxes)
c           gradient values on the tree
c
      implicit real *8 (a-h,o-z)
      integer nd
      integer nlevels
      ...
      real *8 flvals(nd,norder**ndim,*)
      real *8 fl2vals(nd,norder**ndim,*)
      real *8 gvals(nd,ndim,norder**ndim,*)
      real *8 glvals(nd,ndim,norder**ndim,*)
...
c            c6(ilev) = c6(ilev) + d6*dwhts(ndelta-i+1)
      ...
      do ilev = 0,nlevels
         if (idelta(ilev).gt.0) then
C$OMP PARALLEL DO DEFAULT(SHARED)
C$OMP$PRIVATE(ibox,nchild,j,k,ind)
C$OMP$SCHEDULE(DYNAMIC)  
           do ibox = itree(2*ilev+1),itree(2*ilev+2)
              nchild = itree(iptr(4) + ibox-1)
              if(nchild.eq.0) then
...
c     for gradient, always use nasym=2, since calculation of higher
c     derivatives is unstable.
                 do j=1,npbox
                 do k=1,ndim
                 do ind=1,nd
                    grad(ind,k,j,ibox)=grad(ind,k,j,ibox)
     1                  +c0(ilev)*gvals(ind,k,j,ibox)
     2                  +c2(ilev)*glvals(ind,k,j,ibox)
                 enddo
                 enddo
                 enddo
              endif
...
c
c
      subroutine treedata_eval_pgh_nd_asym_fast(ndim,nd,ndelta,deltas,
     1    dwhts,idelta,ipoly,nasym,nlevels,itree,iptr,boxsize,norder,
     2    fvals,flvals,fl2vals,gvals,glvals,hvals,hlvals,
     3    pot,grad,hess)
c
c     This code evaluates the potential, gradient, and hessian at the
c     tensor grid on an adaptive tree
c     using asympotitic expansions with precompted Laplacian and biLaplacian
c 
c     input:
c     ndim - dimension of the underlying space
...
c     glvals - double (nd,ndim,norder**ndim,nboxes)
c           Laplacian of gradient of the function
c     hvals - double (nd,nhess,norder**ndim,nboxes)
c           gradient of function values on the tree
c     hlvals - double (nd,nhess,norder**ndim,nboxes)
c           Laplacian of gradient of the function
c
c     output: incremented value!!!
c     pot - double precision (nd,norder**ndim,nboxes)
c           potential values on the tree
c     grad - double precision (nd,ndim,norder**ndim,nboxes)
c           gradient values on the tree
c     hess - double precision (nd,ndim*(ndim+1)/2,norder**ndim,nboxes)
c           hessian values on the tree
c
      implicit real *8 (a-h,o-z)
      integer nd
      integer nlevels
      ...
      real *8 hvals(nd,ndim*(ndim+1)/2,norder**ndim,*)
      real *8 hlvals(nd,ndim*(ndim+1)/2,norder**ndim,*)

      real *8 pot(nd,norder**ndim,*),grad(nd,ndim,norder**ndim,*)
...
c     for gradient and hessian, always use nasym=2, since calculation of higher
c     derivatives is unstable.
                 do j=1,npbox
                 do k=1,ndim
                 do ind=1,nd
     ...
     2                  +c2(ilev)*glvals(ind,k,j,ibox)
                 ...
                 do k=1,nhess
                 do ind=1,nd
                    hess(ind,k,j,ibox)=hess(ind,k,j,ibox)
     1                  +c0(ilev)*hvals(ind,k,j,ibox)
     2                  +c2(ilev)*hlvals(ind,k,j,ibox)
                 enddo
                 enddo
                 enddo
              endif
