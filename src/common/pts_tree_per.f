...
c     difference between this one and pts_tree.f:
c     add rootboxsize and rootboxcenter to pts_tree_mem and pts_tree_build
...
c 
     ...
     1    ndiv,nlmin,nlmax,ifunif,iper,bs0,cen0,
...
c              eliminates the granddad
               if(nchild(jbox).eq.0.and.iflag(jbox).eq.0) then
                   ict = 0
                   do k=1,ndim
                      dis = abs(centers(k,jbox) - centers(k,idad))
                      if (iper.eq.1) then
                         dp1 = bs0-dis
                         if (dp1.lt. dis) dis = dp1
                      endif
                      if(dis.le.distest) ict = ict + 1
                   enddo
                   if(ict.eq.ndim) then
                      iflag(jbox) = 1
                   endif
...
c                 eliminates the dad
                  if(nchild(jbox).eq.0.and.iflag(jbox).eq.0) then
                     ict = 0
                     do k=1,ndim
                        dis = abs(centers(k,jbox) - centers(k,ibox))
                        if (iper.eq.1) then
                           dp1 = bs0-dis
                           if (dp1.lt. dis) dis = dp1
                        endif
...
c     will take care of handling the flag++ case
         call updateflags(ndim,iper,ilev,nboxes,nlevels,laddr,nchild,
     1       ichild,nnbors,nbors,centers,boxsize,iflag)

         call updateflags(ndim,iper,ilev,nboxes,nlevels,laddrtail,
...
c     Check if kbox is a nearest neighbor or in list 2
                      ...
                      do k=1,ndim
                         ...
                         if(dis.gt.1.05*boxsize(ilev+1)) then
                            ifnbor=0
                            exit
                         endif
                      enddo
