...
c
     ...
     2    iperiod,itree(iptr(6)),itree(iptr(7)))
...
c
c
      subroutine sort_pts_to_uniform_tree_leaf_boxes(ndim,
     1    nlevels,nboxes,ltree,itree,iptr,centers,boxsize,
     2    ibinid,iboxid,ns,sources,isrc,isrcinv,isrcse)
c
...
c
      ...
      integer ibox,mc,mnbors,boxid,ml(0:ndim),ixyz(ndim)
      integer, allocatable :: nsrc(:),isrcbox(:),ip(:)

      integer id,ml0,mlm1,istart,bin
      real *8 bsinv
...
cccc            ixyz(j) = max(ixyz(j),0)
cccc            ixyz(j) = min(ixyz(j),mlm1)
         enddo
         ind = 1
         do j=1,ndim
