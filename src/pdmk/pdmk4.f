...
c     $Revision$
c
c     created on 04/23/2025 by Shidong Jiang
c
c     based on pdmk3.f. changes: change the direct-interaction part to 
c     loop over target box first, then source box. This requires the use of 
c     list1 in the FMM. 
c     The list1 of a box contains its colleagues and neighboring
c     leaf boxes at the coarser and the finer level.
c
...
C$    time2=omp_get_wtime()
      ...
      if (ifprint.eq.1) then
         ...
         call prinf('=== STEP 8 (proxy pot to pot) =============*',i,0)         
         call prinf('=== STEP 9 (direct interactions) ==========*',i,0)         
         call prin2('total time info=*', tottimeinfo,9)
...
c     residual kernel poly expansion coefficients
      ...
      integer istarts,iends,jstarts,jends,istartt,iendt,jstartt,jendt
      integer ilev,jlev,isep
...
C$OMP PARALLEL DO DEFAULT(SHARED)
C$OMP$PRIVATE(ibox,nchild,jbox,j)
C$OMP$SCHEDULE(DYNAMIC)
         do ibox = itree(2*ilev+1),itree(2*ilev+2)
            if (ifpwexp(ibox).eq.1 .and. nboxpts(ibox).gt.0) then
               nb=nb+1
...
C$    time2 = omp_get_wtime()      
     ...
     $     call prinf('number of boxes in local eval=*',nb,1)
      ...
      if(ifprint .ge. 1)
     $    call prinf('=== STEP 5 (proxy potential -> pot) ===*',i,0)
...
C$    time2 = omp_get_wtime()      
 ...
 1800 continue
      if(ifprint .ge. 1)
     $     call prinf('=== STEP 6 (direct interactions) =====*',i,0)
c
cc
      call cpu_time(time1)
C$    time1=omp_get_wtime()
...
C$OMP$PRIVATE(ns,n1,nptssrc,nptstarg,npts,j)
C$OMP$SCHEDULE(DYNAMIC)  
         do ibox = itree(2*ilev+1),itree(2*ilev+2)
