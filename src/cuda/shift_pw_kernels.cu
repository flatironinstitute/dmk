...
template <typename Real>
void launch_shift_pw_multilevel_dispatch(int dim, const std::vector<ShiftPwArgs<Real>> &args_h, cudaStream_t stream) {
    if (dim != 3)
    ...
    void *tmp = nullptr;
    DMK_CHECK_CUDA(cudaMallocAsync(&tmp, args_h.size() * sizeof(ShiftPwArgs<Real>), stream));
    d_args = static_cast<ShiftPwArgs<Real> *>(tmp);

    DMK_CHECK_CUDA(cudaMemcpyAsync(d_args, args_h.data(), args_h.size() * sizeof(ShiftPwArgs<Real>),
                                   cudaMemcpyHostToDevice, stream));

    launch_shift_pw_multilevel_3d<Real>(d_args, static_cast<int>(args_h.size()), max_boxes, stream);

    DMK_CHECK_CUDA(cudaFreeAsync(d_args, stream));
}

template void launch_shift_pw_multilevel_dispatch<float>(int, const std::vector<ShiftPwArgs<float>> &, cudaStream_t);
template void launch_shift_pw_multilevel_dispatch<double>(int, const std::vector<ShiftPwArgs<double>> &, cudaStream_t);
