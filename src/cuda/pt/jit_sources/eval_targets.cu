// V2 eval_targets: evaluates the downward proxy expansion at each eval-box's
// targets (potential, or potential+gradient for EVAL_LEVEL==2). The launcher
// prepends `using Real` + DIM / EVAL_LEVEL / N_CHARGE_DIM / N_ORDER / BLOCK_SIZE
// / TARGETS_PER_THREAD. One launch over the eval-box grid; accumulates into
// pot_flat.

#include <dmk/cuda/eval_targets_kernelargs.hpp>

using dmk::cuda::EvalTargetsArgs;
...
extern "C" __global__ void PtEvalTargetsByBoxKernel(EvalTargetsArgs<Real> a) {
