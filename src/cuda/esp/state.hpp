...
struct GpuState {
    ...
    // so this path tracks the plan's own sigma rather than a baked-in one.
    double beta;
    double self_factor;
    ...
    // Shared-memory sizing for the pruned strategies. The exact max cell population costs a
    // host-device sync, so it is recomputed only when n changes.
    ...
    // Per-eval scratch: grown by byte capacity, never shrunk, reused across calls. Roles used
    // together within one eval share a buffer via sub-offsets.
