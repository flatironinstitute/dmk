...
struct CudaUpwardContext<Real, DIM>::Impl {
    ...
    Impl(DMKPtTree<Real, DIM> &t, CudaSharedDeviceState<Real, DIM> &s) : tree(t), shared(s) {
        ...
        if (s.d_charge_owned == nullptr || s.d_charge_owned_offsets == nullptr)
...
void CudaUpwardContext<Real, DIM>::run() {
    ...
    if (s.proxy_upward_size)
        DMK_CHECK_CUDA(
            cudaMemsetAsync(s.d_proxy_coeffs_upward, 0, s.proxy_upward_size * sizeof(Real), s.downward_stream));
    ...
    if (s.n_c2p_groups) {
        ...
        a.center_boxes = s.d_c2p_center_boxes;
        a.levels = s.d_c2p_levels;
        a.src_box_flat_offsets = s.d_c2p_src_box_flat_offsets;
        a.n_src_boxes_per_group = s.d_c2p_n_src_boxes_per_group;
        a.src_boxes_flat = s.d_c2p_src_boxes_flat;
        a.centers = s.d_centers;
        a.inv_box_scale = s.d_inv_box_scale;
        a.r_src_owned = s.d_r_src_owned;
        a.r_src_owned_offsets = s.d_r_src_owned_offsets;
        a.src_counts_owned = s.d_src_counts_owned;
        a.charge_owned = s.d_charge_owned;
        a.charge_owned_offsets = s.d_charge_owned_offsets;
        a.proxy_flat = s.d_proxy_coeffs_upward;
        a.proxy_offsets = s.d_proxy_offsets_upward;
    ...
    for (int L = s.n_levels - 1; L >= 0; --L) {
        ...
        ta.src_boxes = s.d_tp_up_src_boxes + off;
        ta.dst_boxes = s.d_tp_up_dst_boxes + off;
        ta.child_octants = s.d_tp_up_octants + off;
        ta.proxy_flat = s.d_proxy_coeffs_upward;
        ta.proxy_offsets = s.d_proxy_offsets_upward;
        ta.umat_flat = s.d_c2p;
