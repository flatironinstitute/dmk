...
static void launch_charge2proxy_3d_gemm_micro_ktile_impl(const Charge2ProxyArgs<Real> &args, const int *group_perm,
    ...
    if (err != cudaSuccess)
        throw std::runtime_error(std::string("launch_charge2proxy_3d_gemm_micro_ktile_impl: ") +
...
template <typename Real>
static void launch_charge2proxy_3d(const Charge2ProxyArgs<Real> &args, const int *group_perm, int n_launch_groups,
                                   cudaStream_t stream) {
    if (args.n_groups == 0 || n_launch_groups == 0)
        return;
...
#define DISPATCH_N_ORDER(N)                                                                                            \
    case N:                                                                                                            \
        launch_charge2proxy_3d_gemm_micro_ktile_impl<Real, N, 3, 3, 4>(args, group_perm, n_launch_groups, stream);     \
        break
    ...
    switch (args.n_order) {
        ...
        DISPATCH_N_ORDER(19);
    default:
        throw std::runtime_error("launch_charge2proxy_3d_gemm_micro_ktile: unsupported n_order=" +
