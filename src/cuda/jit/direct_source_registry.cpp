...
int next_direct_kernel_id() { return direct_kernel_counter().fetch_add(1, std::memory_order_relaxed); }
...
const DirectSourceDescriptor &get_direct_source_descriptor(const std::string &kernel_name) {
    ...
    if (it == r.end()) {
        throw std::runtime_error("DirectByBox JIT: no source descriptor registered for " + kernel_name);
    }
...
std::string make_direct_by_box_source(const JitKey &key) {
    const DirectSourceDescriptor &desc = get_direct_source_descriptor(key.name);
    ...
    prelude << "#define DMK_DIRECT_EVALUATOR " << desc.evaluator_expr << "\n\n";
