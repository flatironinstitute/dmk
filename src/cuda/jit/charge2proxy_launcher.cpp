...
Charge2ProxyLaunchConfig default_charge2proxy_config() { return Charge2ProxyLaunchConfig{}; }
...
TuningParams charge2proxy_tuning_params(const Charge2ProxyLaunchConfig &config) {
    return TuningParams{
        {"CHUNK", config.chunk},   {"I_TILE", config.i_tile},        {"J_TILE", config.j_tile},
        {"K_TILE", config.k_tile}, {"BLOCK_SIZE", config.blocksize},
    };
}
...
Charge2ProxyLaunchConfig charge2proxy_config_from_params(const TuningParams &params) {
    ...
    return Charge2ProxyLaunchConfig{
        tuning_param_or(params, "CHUNK", defaults.chunk),          tuning_param_or(params, "I_TILE", defaults.i_tile),
        tuning_param_or(params, "J_TILE", defaults.j_tile),        tuning_param_or(params, "K_TILE", defaults.k_tile),
...
std::size_t charge2proxy_shared_bytes(int n_order, int n_charge_dim, int chunk, std::size_t sizeof_real) {
    ...
    return (std::size_t{3} * std::size_t(n_order) * std::size_t(ld) + std::size_t(n_charge_dim) * std::size_t(ld)) *
           sizeof_real;
}
...
AutotuneDeviceRangeSnapshots<Real> make_charge2proxy_output_snapshots(const dmk::cuda::Charge2ProxyArgs<Real> &args,
                                                                      const int *group_perm, int n_launch_groups,
                                                                      cudaStream_t stream) {
    std::vector<int> groups(static_cast<std::size_t>(n_launch_groups));
    if (group_perm != nullptr) {
        check_cuda(
            cudaMemcpyAsync(groups.data(), group_perm, groups.size() * sizeof(int), cudaMemcpyDeviceToHost, stream),
    ...
    for (std::size_t i = 0; i < groups.size(); ++i) {
        check_cuda(cudaMemcpyAsync(&center_boxes[i], args.center_boxes + groups[i], sizeof(int), cudaMemcpyDeviceToHost,
                                   stream),
    ...
    return make_device_range_snapshots<Real>(std::move(ranges),
                                             charge2proxy_coeff_count(args.n_order, args.n_charge_dim), stream);
}
...
template <typename Real>
void launch_charge2proxy_jit(JitCache &cache, const dmk::cuda::Charge2ProxyArgs<Real> &args, const int *group_perm,
                             int n_launch_groups, cudaStream_t stream, int chunk, int i_tile, int j_tile, int k_tile,
                             int blocksize) {
    ...
    key.params = {
        ...
        {"CHUNK", chunk},          {"I_TILE", i_tile},
        {"J_TILE", j_tile},        {"K_TILE", k_tile},
    ...
    set_dynamic_smem_if_needed(*kernel, shared_bytes, "launch_charge2proxy_jit");

    kernel->launch(dim3(n_launch_groups, 1, 1), dim3(blocksize, 1, 1), shared_bytes, stream, args, group_perm);
}

template <typename Real>
Charge2ProxyLaunchConfig tune_charge2proxy_launch_config(JitCache &cache, const dmk::cuda::Charge2ProxyArgs<Real> &args,
                                                         const int *group_perm, int n_launch_groups,
                                                         cudaStream_t stream) {
    const Charge2ProxyLaunchConfig defaults = default_charge2proxy_config();
    ...
    const std::vector<TuningParameter> space{
        {"CHUNK", {64, 128}}, {"I_TILE", {2, 3, 4}},      {"J_TILE", {2, 3, 4}},
        {"K_TILE", {2, 4}},   {"BLOCK_SIZE", {128, 256}},
    };
    ...
    const auto constraint = [&](const TuningParams &params) {
        ...
        if (config.chunk <= 0 || config.i_tile <= 0 || config.i_tile > args.n_order || config.j_tile <= 0 ||
            config.j_tile > args.n_order || config.k_tile <= 0 || config.k_tile > args.n_order) {
            return false;
        }
        ...
        const std::size_t shared_bytes =
            charge2proxy_shared_bytes(args.n_order, args.n_charge_dim, config.chunk, sizeof(Real));
    ...
    const auto benchmark = [&](const TuningParams &params) {
        const Charge2ProxyLaunchConfig config = charge2proxy_config_from_params(params);
        if (!snapshots) {
            snapshots.emplace(make_charge2proxy_output_snapshots(args, group_perm, n_launch_groups, stream));
        }
        ...
        try {
            const double runtime_ms = benchmark_cuda_ms(stream, options.benchmark, [&](cudaStream_t bench_stream) {
                launch_charge2proxy_jit<Real>(cache, args, group_perm, n_launch_groups, bench_stream, config.chunk,
                                              config.i_tile, config.j_tile, config.k_tile, config.blocksize);
            });
            restore_device_range_snapshots(*snapshots, stream);
            return runtime_ms;
        } catch (...) {
    ...
    const GridTuneDecision decision =
        tune_grid(options, space, charge2proxy_tuning_params(defaults), constraint, benchmark);

    if (snapshots) {
        restore_device_range_snapshots(*snapshots, stream);
    }

    const Charge2ProxyLaunchConfig tuned_config = charge2proxy_config_from_params(decision.params);
    charge2proxy_config_cache()[in_process_key] = tuned_config;
    return tuned_config;
}

template <typename Real>
void launch_charge2proxy_autotuned(JitCache &cache, const dmk::cuda::Charge2ProxyArgs<Real> &args,
                                   const int *group_perm, int n_launch_groups, cudaStream_t stream) {
    Charge2ProxyLaunchConfig config = default_charge2proxy_config();

    try {
        config = tune_charge2proxy_launch_config<Real>(cache, args, group_perm, n_launch_groups, stream);
    ...
    launch_charge2proxy_jit<Real>(cache, args, group_perm, n_launch_groups, stream, config.chunk, config.i_tile,
                                  config.j_tile, config.k_tile, config.blocksize);
}

template void launch_charge2proxy_jit<float>(JitCache &, const dmk::cuda::Charge2ProxyArgs<float> &, const int *, int,
                                             cudaStream_t, int, int, int, int, int);

template void launch_charge2proxy_jit<double>(JitCache &, const dmk::cuda::Charge2ProxyArgs<double> &, const int *, int,
                                              cudaStream_t, int, int, int, int, int);
...
void launch_charge2proxy_3d_impl(const Charge2ProxyArgs<Real> &args, const int *group_perm, int n_launch_groups,
    ...
    dmk::cuda::jit::launch_charge2proxy_autotuned<Real>(jit_cache, args, group_perm, n_launch_groups, stream);
}
