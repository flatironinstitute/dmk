...
TuningParams pw_to_proxy_tuning_params(const PwToProxyLaunchConfig &config) {
    return TuningParams{
        {"K1_TILE", config.k1_tile}, {"COL_REG", config.col_reg}, {"K2_TILE", config.k2_tile},
        {"K3_TILE", config.k3_tile}, {"KR_TILE", config.kr_tile}, {"BLOCK_SIZE", config.blocksize},
    };
}
...
PwToProxyLaunchConfig pw_to_proxy_config_from_params(const TuningParams &params,
    ...
    return PwToProxyLaunchConfig{
        tuning_param_or(params, "K1_TILE", defaults.k1_tile), tuning_param_or(params, "COL_REG", defaults.col_reg),
        tuning_param_or(params, "K2_TILE", defaults.k2_tile), tuning_param_or(params, "K3_TILE", defaults.k3_tile),
        tuning_param_or(params, "KR_TILE", defaults.kr_tile), tuning_param_or(params, "BLOCK_SIZE", defaults.blocksize),
    };
}
...
std::size_t pw_to_proxy_shared_bytes(int max_n_pw, int max_n_pw2, int max_n_order, int k1_tile,
...
void check_pw_to_proxy_shape_or_throw(const dmk::cuda::PwToProxyArgs<Real> &a, const char *where) {
    auto fail = [&](const char *field) {
        throw std::runtime_error(
            std::string("PwToProxy JIT: invalid args/shape in ") + where + " field=" + field +
            " n_boxes_at_level=" + std::to_string(a.n_boxes_at_level) + " n_order=" + std::to_string(a.n_order) +
            " n_pw=" + std::to_string(a.n_pw) + " n_pw2=" + std::to_string(a.n_pw2) +
            " n_charge_dim=" + std::to_string(a.n_charge_dim) + " pw_in_stride=" + std::to_string(a.pw_in_stride) +
    ...
    if (a.box_ids == nullptr)
        fail("box_ids");
    if (a.pw_in_pool == nullptr)
        fail("pw_in_pool");
    if (a.pw2poly == nullptr)
        fail("pw2poly");
    if (a.proxy_flat == nullptr)
        fail("proxy_flat");
    if (a.proxy_offsets == nullptr)
        fail("proxy_offsets");
}
...
template <typename Real>
AutotuneDeviceRangeSnapshots<Real> make_pw_to_proxy_output_snapshots(const dmk::cuda::PwToProxyArgs<Real> &args,
                                                                     cudaStream_t stream) {
    std::vector<int> box_ids(static_cast<std::size_t>(args.n_boxes_at_level));
    check_cuda(
        cudaMemcpyAsync(box_ids.data(), args.box_ids, box_ids.size() * sizeof(int), cudaMemcpyDeviceToHost, stream),
    ...
    for (int box : box_ids) {
        long offset = -1;
        check_cuda(cudaMemcpyAsync(&offset, args.proxy_offsets + box, sizeof(long), cudaMemcpyDeviceToHost, stream),
    ...
    return make_device_range_snapshots<Real>(std::move(ranges),
                                             pw_to_proxy_coeff_count(args.n_order, args.n_charge_dim), stream);
}

template <typename Real>
AutotuneDeviceRangeSnapshots<Real>
make_pw_to_proxy_multilevel_output_snapshots(const std::vector<dmk::cuda::PwToProxyArgs<Real>> &args_h,
                                             cudaStream_t stream) {
    ...
    for (const auto &args : args_h) {
        ...
        auto part = make_device_range_snapshots<Real>(std::move(ranges),
                                                      pw_to_proxy_coeff_count(args.n_order, args.n_charge_dim), stream);
        snapshots.insert(snapshots.end(), std::make_move_iterator(part.begin()), std::make_move_iterator(part.end()));
    }
...
std::string pw_to_proxy_tuning_key(const dmk::cuda::PwToProxyArgs<Real> &args) {
    std::ostringstream ss;
    ss << "PwToProxyKernel"
...
std::string make_pw2proxy_source(const JitKey &key) {
    ...
    if (key.name == "PwToProxyKernel") {
        ...
        throw std::runtime_error("PwToProxy JIT: unknown kernel name: " + key.name);
    }
...
template <typename Real>
void launch_pw_to_proxy_jit(JitCache &cache, const dmk::cuda::PwToProxyArgs<Real> &args, cudaStream_t stream,
                            int k1_tile, int col_reg, int k2_tile, int k3_tile, int kr_tile, int blocksize) {
    if (args.n_boxes_at_level == 0) {
        return;
    }
    ...
    const std::size_t shared_bytes =
        pw_to_proxy_shared_bytes(args.n_pw, args.n_pw2, args.n_order, k1_tile, sizeof(dmk::cuda_helpers::complx<Real>));

    set_dynamic_smem_if_needed(*kernel, shared_bytes, "launch_pw_to_proxy_jit");

    kernel->launch(dim3(args.n_boxes_at_level, 1, 1), dim3(blocksize, 1, 1), shared_bytes, stream, args);
}

template <typename Real>
void launch_pw_to_proxy_multilevel_jit(JitCache &cache, const std::vector<dmk::cuda::PwToProxyArgs<Real>> &args_h,
                                       dmk::cuda::PwToProxyArgs<Real> *d_args_scratch, cudaStream_t stream, int k1_tile,
                                       int col_reg, int k2_tile, int k3_tile, int kr_tile, int blocksize) {
    if (args_h.empty()) {
        return;
    }
    ...
    DMK_CHECK_CUDA(cudaMemcpyAsync(d_args_scratch, args_h.data(),
                                   args_h.size() * sizeof(dmk::cuda::PwToProxyArgs<Real>), cudaMemcpyHostToDevice,
                                   stream));
    ...
    const std::size_t shared_bytes =
        pw_to_proxy_shared_bytes(max_n_pw, max_n_pw2, max_n_order, k1_tile, sizeof(dmk::cuda_helpers::complx<Real>));

    set_dynamic_smem_if_needed(*kernel, shared_bytes, "launch_pw_to_proxy_multilevel_jit");

    const int n_args = static_cast<int>(args_h.size());

    kernel->launch(dim3(max_boxes, n_args, 1), dim3(blocksize, 1, 1), shared_bytes, stream, d_args_scratch, n_args);
}

template void launch_pw_to_proxy_jit<float>(JitCache &, const dmk::cuda::PwToProxyArgs<float> &, cudaStream_t, int, int,
                                            int, int, int, int);

template void launch_pw_to_proxy_jit<double>(JitCache &, const dmk::cuda::PwToProxyArgs<double> &, cudaStream_t, int,
                                             int, int, int, int, int);

template void launch_pw_to_proxy_multilevel_jit<float>(JitCache &, const std::vector<dmk::cuda::PwToProxyArgs<float>> &,
                                                       dmk::cuda::PwToProxyArgs<float> *, cudaStream_t, int, int, int,
                                                       int, int, int);

template void launch_pw_to_proxy_multilevel_jit<double>(JitCache &,
                                                        const std::vector<dmk::cuda::PwToProxyArgs<double>> &,
                                                        dmk::cuda::PwToProxyArgs<double> *, cudaStream_t, int, int, int,
                                                        int, int, int);

template <typename Real, int DIM>
PwToProxyLaunchConfig tune_pw_to_proxy_launch_config(JitCache &cache, const dmk::cuda::PwToProxyArgs<Real> &args,
                                                     cudaStream_t stream) {
    ...
    const std::vector<TuningParameter> space{
        {"K1_TILE", {4, 6, 8}}, {"COL_REG", {1, 2}}, {"K2_TILE", {2, 3}},
        {"K3_TILE", {3}},       {"KR_TILE", {4, 6}}, {"BLOCK_SIZE", {128, 256}},
    };

    const auto constraint = [&](const TuningParams &params) {
        const PwToProxyLaunchConfig config = pw_to_proxy_config_from_params(params, defaults);

        if (config.blocksize <= 0 || config.blocksize > prop.maxThreadsPerBlock || config.blocksize % 32 != 0) {
            return false;
        }
        if (config.k1_tile <= 0 || config.k1_tile > args.n_order || config.col_reg <= 0 || config.k2_tile <= 0 ||
            config.k2_tile > args.n_order || config.k3_tile <= 0 || config.k3_tile > args.n_order ||
            config.kr_tile <= 0 || config.kr_tile > config.k1_tile) {
            return false;
        }
        if (config.k1_tile * config.col_reg > 24 || config.kr_tile * config.k2_tile > 24) {
            return false;
        }

        const std::size_t shared_bytes = pw_to_proxy_shared_bytes(args.n_pw, args.n_pw2, args.n_order, config.k1_tile,
    ...
    const auto benchmark = [&](const TuningParams &params) {
        ...
        try {
            const double runtime_ms = benchmark_cuda_ms(stream, options.benchmark, [&](cudaStream_t bench_stream) {
                launch_pw_to_proxy_jit<Real>(cache, args, bench_stream, config.k1_tile, config.col_reg, config.k2_tile,
                                             config.k3_tile, config.kr_tile, config.blocksize);
            });
            restore_device_range_snapshots(*snapshots, stream);
            return runtime_ms;
        } catch (...) {
    ...
    const GridTuneDecision decision =
        tune_grid(options, space, pw_to_proxy_tuning_params(defaults), constraint, benchmark);

    if (snapshots) {
        restore_device_range_snapshots(*snapshots, stream);
    }

    const PwToProxyLaunchConfig tuned_config = pw_to_proxy_config_from_params(decision.params, defaults);
    pw_to_proxy_config_cache()[in_process_key] = tuned_config;
    return tuned_config;
}

template <typename Real, int DIM>
void launch_pw_to_proxy_autotuned(JitCache &cache, const dmk::cuda::PwToProxyArgs<Real> &args, cudaStream_t stream) {
    PwToProxyLaunchConfig config = default_pw_to_proxy_config(false);

    try {
        config = tune_pw_to_proxy_launch_config<Real, DIM>(cache, args, stream);
    ...
    launch_pw_to_proxy_jit<Real>(cache, args, stream, config.k1_tile, config.col_reg, config.k2_tile, config.k3_tile,
                                 config.kr_tile, config.blocksize);
}

template <typename Real, int DIM>
PwToProxyLaunchConfig
tune_pw_to_proxy_multilevel_launch_config(JitCache &cache, const std::vector<dmk::cuda::PwToProxyArgs<Real>> &args_h,
                                          dmk::cuda::PwToProxyArgs<Real> *d_args_scratch, cudaStream_t stream) {
    ...
    const std::vector<TuningParameter> space{
        {"K1_TILE", {12, 18}}, {"COL_REG", {1, 2}}, {"K2_TILE", {2, 3}},
        {"K3_TILE", {3}},      {"KR_TILE", {6, 9}}, {"BLOCK_SIZE", {128, 256}},
    };
    ...
    const auto constraint = [&](const TuningParams &params) {
        ...
        if (config.k1_tile <= 0 || config.k1_tile > max_n_order || config.col_reg <= 0 || config.k2_tile <= 0 ||
            config.k2_tile > max_n_order || config.k3_tile <= 0 || config.k3_tile > max_n_order ||
            config.kr_tile <= 0 || config.kr_tile > config.k1_tile) {
            return false;
        }
        if (config.k1_tile * config.col_reg > 36 || config.kr_tile * config.k2_tile > 36) {
            return false;
        }

        const std::size_t shared_bytes = pw_to_proxy_shared_bytes(max_n_pw, max_n_pw2, max_n_order, config.k1_tile,
                                                                  sizeof(dmk::cuda_helpers::complx<Real>));
        return shared_bytes <= max_shared_bytes;
    };
    ...
    const auto benchmark = [&](const TuningParams &params) {
        ...
        if (!snapshots) {
            snapshots.emplace(make_pw_to_proxy_multilevel_output_snapshots(args_h, stream));
        }

        restore_device_range_snapshots(*snapshots, stream);
        try {
            const double runtime_ms = benchmark_cuda_ms(stream, options.benchmark, [&](cudaStream_t bench_stream) {
                launch_pw_to_proxy_multilevel_jit<Real>(cache, args_h, d_args_scratch, bench_stream, config.k1_tile,
                                                        config.col_reg, config.k2_tile, config.k3_tile, config.kr_tile,
                                                        config.blocksize);
            });
            restore_device_range_snapshots(*snapshots, stream);
            return runtime_ms;
...
template <typename Real, int DIM>
void launch_pw_to_proxy_multilevel_autotuned(JitCache &cache, const std::vector<dmk::cuda::PwToProxyArgs<Real>> &args_h,
                                             dmk::cuda::PwToProxyArgs<Real> *d_args_scratch, cudaStream_t stream) {
    PwToProxyLaunchConfig config = default_pw_to_proxy_config(true);

    try {
        config = tune_pw_to_proxy_multilevel_launch_config<Real, DIM>(cache, args_h, d_args_scratch, stream);
    } catch (const std::exception &e) {
        if (env_flag_enabled("DMK_JIT_AUTOTUNE_VERBOSE")) {
    ...
    launch_pw_to_proxy_multilevel_jit<Real>(cache, args_h, d_args_scratch, stream, config.k1_tile, config.col_reg,
...
void launch_pw_to_proxy(const PwToProxyArgs<Real> &args, cudaStream_t stream) {
    ...
    dmk::cuda::jit::launch_pw_to_proxy_autotuned<Real, DIM>(jit_cache, args, stream);
}
...
void launch_pw_to_proxy_multilevel(const std::vector<PwToProxyArgs<Real>> &args_h, PwToProxyArgs<Real> *d_args_scratch,
    ...
    dmk::cuda::jit::launch_pw_to_proxy_multilevel_autotuned<Real, DIM>(jit_cache, args_h, d_args_scratch, stream);
}

template void launch_pw_to_proxy_multilevel<float, 2>(const std::vector<PwToProxyArgs<float>> &, PwToProxyArgs<float> *,
                                                      cudaStream_t);
template void launch_pw_to_proxy_multilevel<float, 3>(const std::vector<PwToProxyArgs<float>> &, PwToProxyArgs<float> *,
                                                      cudaStream_t);
