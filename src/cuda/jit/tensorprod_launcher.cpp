...
std::size_t tensorprod_shared_bytes(
    int n_order,
    int z_tile,
...
class DeviceRangeSnapshot {
  public:
    DeviceRangeSnapshot(long offset, std::size_t count, Real* data)
        : offset_(offset), count_(count), data_(data) {}
...
template <typename Real>
TensorprodOutputSnapshots<Real> make_tensorprod_output_snapshots(
    ...
    for (std::size_t i = 0; i < dst_boxes.size(); ++i) {
        check_cuda(
            cudaMemcpyAsync(
                &dst_offsets[i],
                args.proxy_offsets + dst_boxes[i],
                sizeof(long),
                cudaMemcpyDeviceToHost,
                stream
            ),
    ...
    const std::size_t coeff_count =
        tensorprod_coeff_count(args.n_order, args.n_charge_dim);
    ...
    for (long offset : dst_offsets) {
        ...
        void* raw = nullptr;
        check_cuda(
            cudaMalloc(&raw, coeff_count * sizeof(Real)),
            "Tensorprod allocate output snapshot"
        );
        ...
        try {
            check_cuda(
                cudaMemcpyAsync(
                    saved,
                    args.proxy_flat + offset,
                    coeff_count * sizeof(Real),
                    cudaMemcpyDeviceToDevice,
                    stream
                ),
...
template <typename Real>
void restore_tensorprod_output_snapshots(
    const TensorprodOutputSnapshots<Real>& snapshots,
    Real* proxy_flat,
    cudaStream_t stream
) {
...
template <typename Real>
void launch_tensorprod_jit_config(
    JitCache& cache,
    const dmk::cuda::TensorprodArgs<Real>& args,
    cudaStream_t stream,
...
void launch_tensorprod_jit(
    ...
    launch_tensorprod_jit_config<Real>(
        cache,
        args,
        stream,
        default_tensorprod_config(blocksize)
    );
}
...
template <typename Real, int DIM>
TensorprodLaunchConfig tune_tensorprod_launch_config(
    JitCache& cache,
    const dmk::cuda::TensorprodArgs<Real>& args,
    cudaStream_t stream
) {
    ...
    const auto constraint = [&](const TuningParams& params) {
        ...
        if (config.z_tile <= 0 || config.z_tile > args.n_order ||
        ...
        const std::size_t shared_bytes =
            tensorprod_shared_bytes(args.n_order, config.z_tile, sizeof(Real));
    ...
    const GridTuneDecision decision =
        tune_grid(
            options,
            space,
            tensorprod_tuning_params(defaults),
            constraint,
            benchmark
        );
    ...
    const TensorprodLaunchConfig tuned_config =
        tensorprod_config_from_params(decision.params);
    tensorprod_config_cache()[in_process_key] = tuned_config;
    return tuned_config;
}

template <typename Real, int DIM>
void launch_tensorprod_autotuned(
    JitCache& cache,
    const dmk::cuda::TensorprodArgs<Real>& args,
    cudaStream_t stream
) {
    ...
    try {
        config = tune_tensorprod_launch_config<Real, DIM>(
            cache,
            args,
            stream
        );
...
void launch_tensorprod(const TensorprodArgs<Real> &args, cudaStream_t stream) {
    ...
    dmk::cuda::jit::launch_tensorprod_autotuned<Real, DIM>(
        jit_cache,
        args,
        stream
    );
