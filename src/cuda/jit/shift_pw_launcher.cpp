...
std::string shift_pw_tuning_key(
...
template <typename Real, int DIM>
std::string shift_pw_multilevel_tuning_key(
    ...
    if (!args_h.empty()) {
        ss << "|n_neighbors=" << args_h[0].n_neighbors
...
template <typename Real, int DIM>
void launch_shift_pw_autotuned(
    JitCache& cache,
    const dmk::cuda::ShiftPwArgs<Real>& args,
    cudaStream_t stream,
    int default_blocksize
...
int tune_shift_pw_multilevel_blocksize(
    ...
    const std::string tune_key =
        shift_pw_multilevel_tuning_key<Real, DIM>(args_h, max_boxes);
    const bool force = env_flag_enabled("DMK_JIT_AUTOTUNE_FORCE");
    int device = 0;
...
template <typename Real, int DIM>
void launch_shift_pw_multilevel_autotuned(
    JitCache& cache,
    const std::vector<dmk::cuda::ShiftPwArgs<Real>>& args_h,
    dmk::cuda::ShiftPwArgs<Real>* d_args_scratch,
    cudaStream_t stream,
...
void launch_shift_pw(const ShiftPwArgs<Real> &args, cudaStream_t stream) {
    ...
    dmk::cuda::jit::launch_shift_pw_autotuned<Real, DIM>(
        jit_cache,
        args,
        stream,
        block_size
...
void launch_shift_pw_multilevel(
    ...
    dmk::cuda::jit::launch_shift_pw_multilevel_autotuned<Real, DIM>(
        jit_cache,
        args_h,
        d_args_scratch,
        stream,
