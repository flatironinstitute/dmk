...
Proxy2PwLaunchConfig default_proxy2pw_config(int blocksize = PROXY2PW_BLOCK_SIZE) {
    return Proxy2PwLaunchConfig{
        blocksize, PROXY2PW_Z_TILE, PROXY2PW_I_TILE, PROXY2PW_M1_TILE, PROXY2PW_M2_TILE,
    };
}
...
TuningParams proxy2pw_tuning_params(const Proxy2PwLaunchConfig &config) {
    return TuningParams{
        {"BLOCK_SIZE", config.blocksize}, {"Z_TILE", config.z_tile},   {"I_TILE", config.i_tile},
        {"M1_TILE", config.m1_tile},      {"M2_TILE", config.m2_tile},
    };
}
...
Proxy2PwLaunchConfig proxy2pw_config_from_params(const TuningParams &params) {
    ...
    return Proxy2PwLaunchConfig{
        tuning_param_or(params, "BLOCK_SIZE", defaults.blocksize), tuning_param_or(params, "Z_TILE", defaults.z_tile),
        tuning_param_or(params, "I_TILE", defaults.i_tile),        tuning_param_or(params, "M1_TILE", defaults.m1_tile),
...
std::size_t proxy2pw_shared_bytes(int n_order, int n_pw, int z_tile, std::size_t sizeof_real) {
    const std::size_t complex_count =
        std::size_t(z_tile) * (std::size_t(n_order) * std::size_t(n_order) + std::size_t(n_order) * std::size_t(n_pw)) +
...
void check_proxy2pw_shape_or_throw(const dmk::cuda::Proxy2PwArgs<Real> &a) {
    if (a.n_order <= 0 || a.n_pw <= 0 || a.n_pw2 <= 0 || a.n_charge_dim <= 0) {
        throw std::runtime_error("Proxy2Pw JIT: invalid shape");
    }
}
...
std::string proxy2pw_tuning_key(const dmk::cuda::Proxy2PwArgs<Real> &args) {
    std::ostringstream ss;
    ss << "Proxy2PwKernel"
...
std::string make_proxy2pw_source(const JitKey &key) {
    ...
    if (key.name == "Proxy2PwKernel") {
        ...
        throw std::runtime_error("Proxy2Pw JIT: unknown kernel name: " + key.name);
    }
...
template <typename Real>
void launch_proxy2pw_jit_config(JitCache &cache, const dmk::cuda::Proxy2PwArgs<Real> &args, cudaStream_t stream,
    ...
    key.params = {{"BLOCK_SIZE", config.blocksize},
    ...
    const std::size_t shared_bytes = proxy2pw_shared_bytes(args.n_order, args.n_pw, config.z_tile, sizeof(Real));

    set_dynamic_smem_if_needed(*kernel, shared_bytes, "launch_proxy2pw_jit");

    kernel->launch(dim3(args.n_boxes_at_level, 1, 1), dim3(config.blocksize, 1, 1), shared_bytes, stream, args);
}

template <typename Real>
void launch_proxy2pw_jit(JitCache &cache, const dmk::cuda::Proxy2PwArgs<Real> &args, cudaStream_t stream,
                         int blocksize) {
    launch_proxy2pw_jit_config<Real>(cache, args, stream, default_proxy2pw_config(blocksize));
}

template <typename Real>
void launch_proxy2pw_multilevel_jit_config(JitCache &cache, const std::vector<dmk::cuda::Proxy2PwArgs<Real>> &pa_h,
                                           dmk::cuda::Proxy2PwArgs<Real> *d_args_scratch, cudaStream_t stream,
                                           const Proxy2PwLaunchConfig &config) {
    if (pa_h.empty()) {
        return;
    }
    ...
    DMK_CHECK_CUDA(cudaMemcpyAsync(d_args_scratch, pa_h.data(), pa_h.size() * sizeof(dmk::cuda::Proxy2PwArgs<Real>),
    ...
    key.params = {
        {"BLOCK_SIZE", config.blocksize}, {"N_ORDER", pa_h[0].n_order}, {"N_CHARGE_DIM", pa_h[0].n_charge_dim},
        {"N_PW", pa_h[0].n_pw},           {"N_PW2", pa_h[0].n_pw2},     {"Z_TILE", config.z_tile},
        {"I_TILE", config.i_tile},        {"M1_TILE", config.m1_tile},  {"M2_TILE", config.m2_tile}};

    auto kernel = cache.get_kernel(key);

    const std::size_t shared_bytes = proxy2pw_shared_bytes(max_n_order, max_n_pw, config.z_tile, sizeof(Real));

    set_dynamic_smem_if_needed(*kernel, shared_bytes, "launch_proxy2pw_multilevel_jit");

    const int n_args = static_cast<int>(pa_h.size());

    kernel->launch(dim3(max_boxes, n_args, 1), dim3(config.blocksize, 1, 1), shared_bytes, stream, d_args_scratch,
                   n_args);
}

template <typename Real>
void launch_proxy2pw_multilevel_jit(JitCache &cache, const std::vector<dmk::cuda::Proxy2PwArgs<Real>> &pa_h,
                                    dmk::cuda::Proxy2PwArgs<Real> *d_args_scratch, cudaStream_t stream, int blocksize) {
    launch_proxy2pw_multilevel_jit_config<Real>(cache, pa_h, d_args_scratch, stream,
                                                default_proxy2pw_config(blocksize));
}

template void launch_proxy2pw_jit<float>(JitCache &, const dmk::cuda::Proxy2PwArgs<float> &, cudaStream_t, int);

template void launch_proxy2pw_jit<double>(JitCache &, const dmk::cuda::Proxy2PwArgs<double> &, cudaStream_t, int);

template void launch_proxy2pw_multilevel_jit<float>(JitCache &, const std::vector<dmk::cuda::Proxy2PwArgs<float>> &,
                                                    dmk::cuda::Proxy2PwArgs<float> *, cudaStream_t, int);

template void launch_proxy2pw_multilevel_jit<double>(JitCache &, const std::vector<dmk::cuda::Proxy2PwArgs<double>> &,
                                                     dmk::cuda::Proxy2PwArgs<double> *, cudaStream_t, int);

template <typename Real, int DIM>
Proxy2PwLaunchConfig tune_proxy2pw_launch_config(JitCache &cache, const dmk::cuda::Proxy2PwArgs<Real> &args,
                                                 cudaStream_t stream) {
    const Proxy2PwLaunchConfig defaults = default_proxy2pw_config();
    ...
    const std::vector<TuningParameter> space{
        {"BLOCK_SIZE", {128, 256}}, {"Z_TILE", {2, 4}}, {"I_TILE", {2, 4}}, {"M1_TILE", {2, 4, 6}}, {"M2_TILE", {2, 4}},
    };
    ...
    const auto constraint = [&](const TuningParams &params) {
        ...
        if (config.z_tile <= 0 || config.z_tile > args.n_pw2 || config.i_tile <= 0 || config.i_tile > args.n_order ||
            config.m1_tile <= 0 || config.m1_tile > args.n_pw || config.m2_tile <= 0 || config.m2_tile > args.n_pw) {
            return false;
        }
        if (config.i_tile * config.m2_tile > 24 || config.m1_tile * config.m2_tile > 24) {
            return false;
        }
    ...
    const auto benchmark = [&](const TuningParams &params) {
        ...
        return benchmark_cuda_ms(stream, options.benchmark, [&](cudaStream_t bench_stream) {
            launch_proxy2pw_jit_config<Real>(cache, args, bench_stream, config);
        });
    };

    const GridTuneDecision decision =
        tune_grid(options, space, proxy2pw_tuning_params(defaults), constraint, benchmark);

    const Proxy2PwLaunchConfig tuned_config = proxy2pw_config_from_params(decision.params);
    proxy2pw_config_cache()[in_process_key] = tuned_config;
    return tuned_config;
}

template <typename Real, int DIM>
void launch_proxy2pw_autotuned(JitCache &cache, const dmk::cuda::Proxy2PwArgs<Real> &args, cudaStream_t stream) {
    Proxy2PwLaunchConfig config = default_proxy2pw_config();

    try {
        config = tune_proxy2pw_launch_config<Real, DIM>(cache, args, stream);
    ...
    launch_proxy2pw_jit_config<Real>(cache, args, stream, config);
}

template <typename Real, int DIM>
Proxy2PwLaunchConfig
tune_proxy2pw_multilevel_launch_config(JitCache &cache, const std::vector<dmk::cuda::Proxy2PwArgs<Real>> &pa_h,
                                       dmk::cuda::Proxy2PwArgs<Real> *d_args_scratch, cudaStream_t stream) {
    const Proxy2PwLaunchConfig defaults = default_proxy2pw_config();
    if (env_flag_enabled("DMK_JIT_AUTOTUNE_DISABLE")) {
        return defaults;
    ...
    const auto constraint = [&](const TuningParams &params) {
        ...
        if (config.z_tile <= 0 || config.z_tile > max_n_pw2 || config.i_tile <= 0 || config.i_tile > max_n_order ||
            config.m1_tile <= 0 || config.m1_tile > max_n_pw || config.m2_tile <= 0 || config.m2_tile > max_n_pw) {
            return false;
        }
        if (config.i_tile * config.m2_tile > 24 || config.m1_tile * config.m2_tile > 24) {
            return false;
    ...
    const auto benchmark = [&](const TuningParams &params) {
        const Proxy2PwLaunchConfig config = proxy2pw_config_from_params(params);
        return benchmark_cuda_ms(stream, options.benchmark, [&](cudaStream_t bench_stream) {
            launch_proxy2pw_multilevel_jit_config<Real>(cache, pa_h, d_args_scratch, bench_stream, config);
        });
    };
...
template <typename Real, int DIM>
void launch_proxy2pw_multilevel_autotuned(JitCache &cache, const std::vector<dmk::cuda::Proxy2PwArgs<Real>> &pa_h,
                                          dmk::cuda::Proxy2PwArgs<Real> *d_args_scratch, cudaStream_t stream) {
    Proxy2PwLaunchConfig config = default_proxy2pw_config();

    try {
        config = tune_proxy2pw_multilevel_launch_config<Real, DIM>(cache, pa_h, d_args_scratch, stream);
    } catch (const std::exception &e) {
        if (env_flag_enabled("DMK_JIT_AUTOTUNE_VERBOSE")) {
    ...
    launch_proxy2pw_multilevel_jit_config<Real>(cache, pa_h, d_args_scratch, stream, config);
}
...
void launch_proxy2pw(const Proxy2PwArgs<Real> &args, cudaStream_t stream) {
    ...
    dmk::cuda::jit::launch_proxy2pw_autotuned<Real, DIM>(jit_cache, args, stream);
}
...
void launch_proxy2pw_multilevel(const std::vector<Proxy2PwArgs<Real>> &pa_h, Proxy2PwArgs<Real> *d_args_scratch,
    ...
    dmk::cuda::jit::launch_proxy2pw_multilevel_autotuned<Real, DIM>(jit_cache, pa_h, d_args_scratch, stream);
}
