...
__attribute__((always_inline)) void dispatch_digits(int n_digits, F &&f, Args &&...args) {
    switch (n_digits) {
    case 2:
...
__attribute__((always_inline)) VecType blocked_horner(const VecType &x, const typename VecType::ScalarType *coeffs,
                                                      int n_coeffs) {
    VecType x2 = x * x;
    VecType x4 = x2 * x2;
...
template <typename VecType>
__attribute__((always_inline)) VecType horner(const VecType &x, const typename VecType::ScalarType *coeffs,
                                              int n_coeffs) {
...
void EvalPairs(int Ns, const Real *__restrict__ r_src, const Real *__restrict__ v_src,
    ...
    if (NNt == VecLen) {
        ...
        for (Integer k = 0; k < KERNEL_OUTPUT_DIM; k++) {
            ...
            for (Long t = 0; t < Nt; t++) {
                v_trg[t * KERNEL_OUTPUT_DIM + k] += out[t] // * uKernelEvaluator::template uKerScaleFactor<Real>()
                    ;
            }
        }
    } else {
        ...
        for (Long k = 0; k < KERNEL_OUTPUT_DIM; k++) { // v_trg += Vt_
            for (Long i = 0; i < Nt; i++) {
...
void laplace_pswf_all_pairs_jit_impl(int nd, Real rsc, Real cen, Real d2max, Real thresh2, int n_coeffs,
    ...
    VecType thresh2_vec = thresh2;
    VecType d2max_vec = d2max;
    VecType rsc_vec = rsc;
    VecType cen_vec = cen;

    auto evaluator = [thresh2_vec, d2max_vec, rsc_vec, cen_vec, coeffs, n_coeffs](VecType(&u)[1][1],
                                                                                  const VecType(&dX)[3]) {
    ...
    constexpr Real scale_factor = Real{1.0};
