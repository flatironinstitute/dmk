...
int find_interval_index(const std::vector<int> &indices, const int index) {
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        double pos = box_index_grid[n_boxes] * h + r_cut;
        int new_ind = (int(pos * N) + 1) % N;
    ...
    for (size_t ind = 0; ind < n_src; ++ind) {
        ...
        const int i_x = find_interval_index(box_index_grid, int(x * N));
        const int i_y = find_interval_index(box_index_grid, int(y * N));
        const int i_z = find_interval_index(box_index_grid, int(z * N));
    ...
    for (size_t box = 0; box < n_boxes * n_boxes * n_boxes; ++box) {
        ...
        const int box_x = (box % (n_boxes * n_boxes)) % n_boxes;
        const int box_y = (box % (n_boxes * n_boxes)) / n_boxes;
        const int box_z = box / (n_boxes * n_boxes);
        ...
        for (int k = box_z - 1; k <= box_z + 1; ++k) {
            ...
            const int new_i = (i + n_boxes) % n_boxes;
            const int new_j = (j + n_boxes) % n_boxes;
            const int new_k = (k + n_boxes) % n_boxes;
            // for each neighbor compute all the pairwise interactions
            const int curr_box = new_i + new_j * n_boxes + new_k * n_boxes * n_boxes;
    ...
    for (size_t box = 0; box < n_boxes * n_boxes * n_boxes; ++box) {
        ...
        for (int i = 0; i < box_lengths[box]; ++i) {
            ...
            // column-major
            const double x = r_src_sorted[particle_count];
            const double y = r_src_sorted[n_src + particle_count];
            const double z = r_src_sorted[n_src * 2 + particle_count];
