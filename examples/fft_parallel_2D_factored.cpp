/* Final parallel 2D FFT implementation with Alltoall() operations. */
...
void compute_distributed_fft(
    ...
    for (int i = 0; i < nrows; ++i) {
        std::span<const std::complex<Real>> in_col(real_matrix.data() + i * N, N);
        std::span<std::complex<Real>>       out_col(transformed_matrix.data() + i * N, N);
...
int main(int argc, char* argv[]) {
    ...
    compute_distributed_fft(x_slab, ft1_slab, 1, rows[rank], N, true);
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    compute_distributed_fft(ft1_T, ft2_slab, 1, rows[rank], N, true);
    MPI_Barrier(MPI_COMM_WORLD);
