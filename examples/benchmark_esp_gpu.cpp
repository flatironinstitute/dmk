...
// Gauged (mean-subtracted) on both sides: ESP potentials for a charge-neutral system carry an
// arbitrary additive constant, so an ungauged comparison reports spurious error.
template <typename Real>
...
// Times n_runs of esp_eval_gpu; unless -S, one CPU call serves as both speedup baseline and
// accuracy reference.
template <typename Real>
void run_phase(const Config &cfg, int n, const std::vector<dmk::Vec3T<Real>> &r_src, const std::vector<Real> &charges,
    ...
    // Brackets only the timed loop, so `nsys/ncu -c cudaProfilerApi` excludes plan creation,
    // warmup, and the CPU baseline.
...
Config parse_args(int argc, char *argv[]) {
    ...
    while ((opt = getopt(argc, argv, "N:L:c:e:r:t:s:pP:M:FSgh?")) != -1) {
        ...
        case 'p':
            ...
            break;
        case 'P': {
            const int p = std::atoi(optarg);
            if (p < 0 || p > 2)
                throw std::runtime_error(
                    "benchmark_esp_gpu: -P must be 0 (dense), 1 (prune_tile), or 2 (prune_source)");
...
#else // DMK_GPU_OFFLOAD
#include <iostream>
int main() {
    std::cerr << "benchmark_esp_gpu requires -DDMK_GPU_OFFLOAD=ON at configure time.\n";
    return 1;
}
#endif
