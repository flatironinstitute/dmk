...
class TestCaseSystem {
  public:
    ...
    TestCaseSystem(int n_sources, int n_targets, int n_dimensions, Real *r_sources,
                   Real *r_targets, Real *charge, Real length)
...
template <typename Real>
ShortRangeSystem<Real> initialize_short_range_esp(const TestCaseSystem<Real> &System, Real r_cut, int N,
                                              int n_dim) {
    ...
    for (size_t ind = 0; ind < n_src; ++ind) {
        ...
        //convert real coordinates to box indices, to identify which box the particle belongs to
        const int i_x = int(x / L * N) / bin_size_x; 
        const int i_y = int(y / L * N) / bin_size_x;
        const int i_z = int(z / L * N) / bin_size_x;

        ++Short_setup.box_lengths[i_x + nbins_x * (i_y + nbins_x * i_z)]; //increment the counter for the current box
    }

    int current_offset = 0;
    for (size_t i = 0; i < n_boxes; ++i) {
        const int tmp = Short_setup.box_lengths[i];
        Short_setup.box_begin[i] = current_offset; //the starting index in the sorted array where the particles of the current box will be stored
    ...
    //The particles indices are reordered to match the box ordering
    for (size_t ind = 0; ind < n_src; ++ind) {
        // particle coordinates
        const Real x = r_src[ind];
        const Real y = r_src[n_src + ind];
        ...
        Short_setup.particles_sorted[box_offset[i_x + nbins_x * (i_y + nbins_x * i_z)]] = ind; //particles_sorted contains particle indices grouped by box 
        ++box_offset[i_x + nbins_x * (i_y + nbins_x * i_z)];
    }
    //The particles positions and charges are reordered to match the box ordering
    for (size_t i = 0; i < n_src; ++i) {
...
void compute_potential(Real *pot, const Real *x, const Real *y, const Real *z, const Real *charges, int n_particles,
                       ...
                       const Real *offset, Real r_cut_sq, const dmk::Prolate0Fun &pswf, Real c0, Real r_cut) {
    ...
    for (int i = 0; i < n_particles; ++i) {
        ...
        for (int j = 0; j < n_other; ++j) {
            ...
            const Real rij_mag = std::sqrt(rij_mag_sq);
            const Real t = rij_mag / r_cut;  // normalized to [0,1]
            const Real phi = pswf.int_eval(t) / c0;  // c0 = normalization constant = \int_0^1 \psi_0^c(x) dx
            pot[i] += charges[j] * (1.0 - phi) / rij_mag; 
        }
    }
}
...
template <typename Real>
std::vector<Real> evaluate_short_range_esp(const TestCaseSystem<Real> &System, ShortRangeSystem<Real> &Short, Real r_cut, 
    bool vectorized, Real c0, dmk::Prolate0Fun pswf) {
    ...
    for (size_t box = 0; box < n_boxes; ++box) {
        ...
        for (int nb : Short.box_neighbors[box]) {
            ...
            compute_potential(pot_part, x, y, z, ch, Short.box_lengths[box], x_other, y_other, z_other,
                            Short.box_lengths[nb], &(offset[0]), r_cut_sq, pswf, c0, r_cut);
            //     }
            // else {
...
void pme_poisson3d_lagrange(
        ...
        int P,
        int uniform = true,
        int vectorized = false,
        Real *r_sources = nullptr,
...
/*
// Temporarily disabled test runner to allow building while tests are adapted.
