...
template <typename Real>
void init_test_data(MPI_Comm comm, int n_dim, int nd, int n_src, int n_trg, bool uniform, bool set_fixed_charges,
    ...
    MPI_Scatter(&r_src[0], n_src_per_rank * n_dim, mpi_type, &r_src[0], n_src_per_rank * n_dim, mpi_type, 0, comm);
    MPI_Scatter(&charges[0], n_src_per_rank * nd, mpi_type, &charges[0], n_src_per_rank * nd, mpi_type, 0, comm);
...
#pragma omp parallel
    ...
    init_test_data(MPI_COMM_WORLD, n_dim, 1, n_src, n_trg, uniform, set_fixed_charges, r_src, r_trg, rnormal, charges,
                   dipstr, 0);
    ...
    if (rank == 0) {
        std::vector<Real> r_src, r_trg, rnormal, charges, dipstr;
        pot_single.resize(n_src);
        init_test_data(MPI_COMM_SELF, n_dim, 1, n_src, n_trg, uniform, set_fixed_charges, r_src, r_trg, rnormal,
                       charges, dipstr, 0);
        pdmk_tree tree;
        if constexpr (std::is_same_v<Real, float>)
            tree =
                pdmk_tree_createf(MPI_COMM_SELF, params, n_src, &r_src[0], &charges[0], nullptr, nullptr, 0, nullptr);
        else
    ...
    if (rank == 0) {
        ...
        for (int i = 0; i < pot_mpi.size(); ++i)
            if (std::abs((pot_mpi[i] - pot_single[i]) / pot_single[i]) > 10 * params.eps)
                printf("%11d %+12.5e %+12.5e %+12.5e %+12.5e\n", i, pot_single[i], pot_mpi[i], pot_direct_mpi[i],
    ...
    while ((opt = getopt(argc, argv, "N:n:l:e:t:uh?")) != -1) {
        ...
        default:
            std::cout << "Usage: " << argv[0]
                      << " [-N n_src] [-n n_per_leaf_dmk] [-e eps_dmk] [-l log_level] "
