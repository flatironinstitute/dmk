...
C$     t2 = omp_get_wtime()      
      ...
      ntot=npbox*nlfbox+ntarg*ifpghtarg
      pps=(ntot+0.0d0)/(t2-t1)
      call prinf_long('ntotal=*',ntot,1)
...
cccc         call prin2('absolute pot l2 error=*',abserrp,1)
     ...
     1       iptr,boxsize,norder,npbox,fvals,rnormf,nleaf)
         call prin2('rhs l2 norm=*',rnormf,1)

         call treedata_derror(nd,nlevels,itree,iptr,
     1       npbox,fltrue,flvals,abserrfl,rnormfl,nleaf)
         errfl=abserrfl/rnormfl
         call prin2('flap l2 norm=*',rnormfl,1)
         call prin2('relative flap l2 error=*',errfl,1)

         call treedata_derror(nd,nlevels,itree,iptr,
     1       npbox,fl2true,fl2vals,abserrfl2,rnormfl2,nleaf)
         errfl2=abserrfl2/rnormfl2
         call prin2('fbilap l2 norm=*',rnormfl2,1)
         call prin2('relative fbilap l2 error=*',errfl2,1)
         endif
      endif
...
c
      ...
      complex *16 zpars(*)
      real *8 dpars(*),f(nd),xyz(*),f2(nd),pi
...
c     number of Gaussians
     ...
     1              *(-2*ndim+4*rr/sigma)/sigma
      ...
      if (ikernel.eq.0) then
         call uexact(nd,xyz,dpars,zpars,ipars,f2)
         if (ndim.eq.3) then
            dfac=-4*pi
         elseif (ndim.eq.2) then
            if (ikernel.eq.0) then
...
cccc         call vdTGamma(1,alpha/2,gval)
     ...
     1             (log(1.0d0/eps)+ndim*log(1.0d0/sigma)/2)
               ...
               do k=mmin,mmax
                  ...
                  pot(ind)=pot(ind)+d1*d2*h*dpars(idp+5)/gval
               enddo
            enddo
         enddo
