"""
Reusable utilities for loading dumped DMKPtTree state.

Tree dumps are produced when DMK_DEBUG_DUMP_TREE is set. Files use the layout
    [int64_t dimensions][uint64_t n_elems][T data ...]
and are named  '<name>.<comm_size>.<rank>.dat'. A separate
'dmk_params.<comm_size>.dat' header file describes dim / n_order / n_pw / fp.

GPU dumps (when eval_path is GPU or BOTH) live under a 'gpu/' subdirectory so
they don't clobber the CPU dumps. Pass `prefix='gpu'` to read them.

Typical use:
    cpu = DMKPtTreeData(prefix='.', comm_size=1)
    gpu = DMKPtTreeData(prefix='gpu', comm_size=1)
    for mid in cpu.mids[0]:
        compare_coeffs(cpu.get_proxy_coeffs_downward_by_mid(mid)[0],
                       gpu.get_proxy_coeffs_downward_by_mid(mid),
                       'proxy_down')
"""
...
def read_sctl_binary(filename, dtype):
    """Read an SCTL-style dump: skip 16-byte header, return flat array of `dtype`."""
...
def read_params_data(filename):
    ...
    real_dtype = np.float32 if float_size == 4 else np.float64
...
class DMKPtTreeData:
    ...
    ----------
    prefix : str
        Directory containing the .dat files. Use '.' for the standard CPU
        dump location and 'gpu' for the GPU-side dump in eval_path=BOTH mode.
    comm_size : int
    ...
    def __init__(self, prefix='.', comm_size=1):
        ...
        self.n_dim, self.n_order, self.n_pw, self.real_dtype = read_params_data(
            os.path.join(prefix, f'dmk_params.{comm_size}.dat'))
    ...
    def _centers(self):
        return [out.reshape(-1, self.n_dim)
                for out in self._read_all_ranks('dmk_centers', self.real_dtype)]
    ...
    def _pw_out(self):
        return [out.reshape(-1, *self.pw_shape)
                for out in self._read_all_ranks('dmk_pw_out', self.real_dtype)]

    def _pw_out_offsets(self):
        pw_size = np.prod(self.pw_shape)
        raw = self._read_all_ranks('dmk_pw_out_offsets', np.int64)
        for rank in range(self.comm_size):
            raw[rank] = np.array([int(el // pw_size) if el != -1 else int(el)
                                  for el in raw[rank]])
        return raw

    def _proxy_coeffs(self):
        return [out.reshape(-1, *self.proxy_shape)
                for out in self._read_all_ranks('dmk_proxy_coeffs', self.real_dtype)]
    ...
    def _proxy_coeffs_offsets(self):
        ...
        for rank in range(self.comm_size):
            raw[rank] = np.array([int(el // proxy_size) if el != -1 else int(el)
                                  for el in raw[rank]])
        return raw

    def _proxy_coeffs_downward(self):
        return [out.reshape(-1, *self.proxy_shape)
                for out in self._read_all_ranks('dmk_proxy_coeffs_downward', self.real_dtype)]
    ...
    def _morton_ids(self):
        ...
        for rank in range(self.comm_size):
            results.append(
                read_morton_ids(self._filename('dmk_morton_ids', rank), self.n_dim))
        return results

    def __repr__(self):
        return ("DMKPtTreeData(prefix='{}', comm_size={}, dim={}, n_order={}, "
                "n_pw={}, real_dtype={})").format(
                    self.prefix, self.comm_size, self.n_dim,
                    self.n_order, self.n_pw, self.real_dtype)


def compare_coeffs(coeffs_baseline, coeffs_other, tag, mid=None, rtol=1e-7):
    """
    Compare baseline coefficients (single ndarray) against a list of
    candidate ndarrays (per-rank, per-source). Prints any (tag, mid, err)
    above `rtol`.
    """
    if coeffs_baseline is None:
        return

    for coeffs in coeffs_other:
        if coeffs is None:
            continue
        mask = coeffs_baseline != 0
        if not np.any(mask):
            if np.any(coeffs):
                print(tag, mid, "nonzero coeffs where baseline is zero")
            continue
