...
namespace SCTL_NAMESPACE {
  ...
  template <class ValueType, Integer N = DefaultVecLen<ValueType>()> class alignas(sizeof(ValueType) * N) Vec {
    public:
      ...
      template <class MaskType> friend void mask_compress_store(const MaskType& v, const Vec<typename MaskType::ScalarType, MaskType::Size> a, typename MaskType::ScalarType* s);
      template <class MaskType> friend Vec<typename MaskType::ScalarType, MaskType::Size> mask_expand_load(const MaskType& v, const Vec<typename MaskType::ScalarType, MaskType::Size> a, typename MaskType::ScalarType const* s);
      ...
      template <Integer digits, class RealVec> friend RealVec approx_erfc(const RealVec& x);
  ...
  template <class MaskType> unsigned mask_popcnt(const MaskType& a) {
    return mask_popcnt_intrin(a);
  }
  template <class MaskType> void mask_compress_store(const MaskType& v, const Vec<typename MaskType::ScalarType, MaskType::Size> a, typename MaskType::ScalarType* s) {
    mask_compress_store_intrin(v, a.v, s);
  }
  template <class MaskType> Vec<typename MaskType::ScalarType, MaskType::Size> mask_expand_load(const MaskType& v, const Vec<typename MaskType::ScalarType, MaskType::Size> a, typename MaskType::ScalarType const* s) {
    return mask_expand_load_intrin(v, a.v, s);
  }
  ...
  template <Integer digits, class RealVec> RealVec approx_erfc(const RealVec& x) {
    constexpr Integer ORDER = digits;
    if (digits == -1 || ORDER > 13) return erfc(x);
    else return erfc(x);
  }
