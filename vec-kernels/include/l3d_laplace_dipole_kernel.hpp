...
static inline void l3d_eval_poly_der(const Vec &x, const Real (&coefs)[N], Vec &p, Vec &dp) {
    p = Vec(coefs[N - 1]);
    dp = Vec::Zero();
...
static inline Vec l3d_eval_poly_dyn(const Vec &x, const Real *coefs, int ncoefs) {
...
template <class Real, sctl::Integer MaxVecLen = sctl::DefaultVecLen<Real>(), sctl::Integer digits = -1,
          sctl::Integer ndim>
void l3d_local_kernel_directdp_vec_cpp_helper(const int32_t *nd, const Real *rsc, const Real *cen, const Real *d2max,
    ...
    sctl::Matrix<Real> Vs_(Nsrc, nd_ * COORD_DIM, sctl::Ptr2Itr<Real>((Real *)dipvec, nd_ * COORD_DIM * Nsrc), false);
...
#pragma omp parallel for schedule(static)
    for (sctl::Long t = 0; t < Ntrg_; t += VecLen) {
        ...
        for (sctl::Long s = 0; s < Nsrc; s++) {
            ...
            for (long i = 0; i < nd_; i++) {
                Vec dotp = dX[0] * Vec::Load1(&Vs_[s][0 * nd_ + i]) + dX[1] * Vec::Load1(&Vs_[s][1 * nd_ + i]);
                if (COORD_DIM > 2) {
...
void l3d_local_kernel_directdp_vec_cpp(const int32_t *nd, const int32_t *ndim, const int32_t *digits, const Real *rsc,
                                       const Real *cen, const Real *d2max, const Real *sources, const int32_t *ns,
                                       const Real *dipvec, const Real *xtarg, const Real *ytarg, const Real *ztarg,
                                       const int32_t *nt, Real *pot, const Real *thresh) {
    if (ndim[0] == 3) {
        if (digits[0] <= 3)
            l3d_local_kernel_directdp_vec_cpp_helper<Real, MaxVecLen, 3, 3>(nd, rsc, cen, d2max, sources, ns, dipvec,
                                                                            xtarg, ytarg, ztarg, nt, pot, thresh);
        else if (digits[0] <= 6)
            l3d_local_kernel_directdp_vec_cpp_helper<Real, MaxVecLen, 6, 3>(nd, rsc, cen, d2max, sources, ns, dipvec,
                                                                            xtarg, ytarg, ztarg, nt, pot, thresh);
        else if (digits[0] <= 9)
            l3d_local_kernel_directdp_vec_cpp_helper<Real, MaxVecLen, 9, 3>(nd, rsc, cen, d2max, sources, ns, dipvec,
                                                                            xtarg, ytarg, ztarg, nt, pot, thresh);
        else
            l3d_local_kernel_directdp_vec_cpp_helper<Real, MaxVecLen, 12, 3>(nd, rsc, cen, d2max, sources, ns, dipvec,
                                                                             xtarg, ytarg, ztarg, nt, pot, thresh);
    }
}
...
template <class Real, sctl::Integer MaxVecLen = sctl::DefaultVecLen<Real>(), sctl::Integer digits = -1,
          sctl::Integer ndim>
void l3d_local_kernel_directcp_coef_vec_cpp_helper(const int32_t *nd, const Real *rsc, const Real *cen,
                                                   const Real *d2max, const Real *sources, const int32_t *ns,
                                                   const Real *charge, const Real *xtarg, const Real *ytarg,
                                                   const Real *ztarg, const int32_t *nt, const int32_t *ncoefs,
                                                   const Real *coefs, Real *pot, const Real *thresh) {
    static constexpr sctl::Integer COORD_DIM = ndim;
    constexpr sctl::Long nd_ = 1;
    sctl::Long Nsrc = ns[0];
    sctl::Long Ntrg = nt[0];
...
template <class Real, sctl::Integer MaxVecLen = sctl::DefaultVecLen<Real>()>
void l3d_local_kernel_directcp_coef_vec_cpp(const int32_t *nd, const int32_t *ndim, const int32_t *digits,
                                            const Real *rsc, const Real *cen, const Real *d2max, const Real *sources,
    ...
    if (ndim[0] == 3) {
        if (digits[0] <= 3)
            l3d_local_kernel_directcp_coef_vec_cpp_helper<Real, MaxVecLen, 3, 3>(
                nd, rsc, cen, d2max, sources, ns, charge, xtarg, ytarg, ztarg, nt, ncoefs, coefs, pot, thresh);
        else if (digits[0] <= 6)
...
template <class Real, sctl::Integer MaxVecLen = sctl::DefaultVecLen<Real>(), sctl::Integer digits = -1,
          sctl::Integer ndim>
void l3d_local_kernel_directdp_coef_vec_cpp_helper(const int32_t *nd, const Real *rsc, const Real *cen,
                                                   const Real *d2max, const Real *sources, const int32_t *ns,
                                                   const Real *dipvec, const Real *xtarg, const Real *ytarg,
                                                   const Real *ztarg, const int32_t *nt, const int32_t *ncoefs,
                                                   const Real *coefs, const int32_t *ncoefsd, const Real *coefsd,
...
#pragma omp parallel for schedule(static)
    for (sctl::Long t = 0; t < Ntrg_; t += VecLen) {
        ...
        for (sctl::Long s = 0; s < Nsrc; s++) {
            ...
            Vec xtmp = FMA(R2, Rinv, cen_vec) * rsc_vec;
            Vec dptmp = l3d_eval_poly_dyn<Vec, Real>(xtmp, coefsd, ncoefsd[0]);
...
template <class Real, sctl::Integer MaxVecLen = sctl::DefaultVecLen<Real>()>
void l3d_local_kernel_directdp_coef_vec_cpp(const int32_t *nd, const int32_t *ndim, const int32_t *digits,
                                            const Real *rsc, const Real *cen, const Real *d2max, const Real *sources,
                                            const int32_t *ns, const Real *dipvec, const Real *xtarg, const Real *ytarg,
                                            const Real *ztarg, const int32_t *nt, const int32_t *ncoefs,
                                            const Real *coefs, const int32_t *ncoefsd, const Real *coefsd, Real *pot,
                                            const Real *thresh) {
    if (ndim[0] == 3) {
        if (digits[0] <= 3)
            l3d_local_kernel_directdp_coef_vec_cpp_helper<Real, MaxVecLen, 3, 3>(
                nd, rsc, cen, d2max, sources, ns, dipvec, xtarg, ytarg, ztarg, nt, ncoefs, coefs, ncoefsd, coefsd, pot,
                thresh);
        else if (digits[0] <= 6)
