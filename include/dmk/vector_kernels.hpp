...
void l3d_near_kernel_directcp_vec_cpp_helper(const int32_t *nd, const Real *rsc, const Real *cen, const Real *bsizeinv,
    ...
    constexpr auto coefs = []() {
        if (digits <= 3) {
            return std::array<Real, 9>{2.146941209293174e-02,  -6.263795886222773e-02, 8.000991991853681e-02,
    ...
    for (sctl::Long t = 0; t < Ntrg_; t += VecLen) {
        ...
        for (sctl::Long s = 0; s < Nsrc; s++) {
            ...
            Vec rtmp = EvalPolynomial(xtmp.get(), coefs);
...
void sl3d_near_kernel_directcp_vec_cpp_helper(const int32_t *nd, const Real *rsc, const Real *cen, const Real *bsizeinv,
    ...
    constexpr auto coefs = []() {
        if constexpr (digits <= 3) {
            return std::array<Real, 9>{8.173887886459996e-02,  -2.451274687320377e-01, 3.277193048641895e-01,
...
void log_near_kernel_directcp_vec_cpp_helper(const int32_t *nd, const Real *rsc, const Real *cen, const Real *bsizeinv,
    ...
    constexpr auto coefs = []() {
        if constexpr (digits <= 3) {
            return std::array<Real, 9>{-6.111367193489408e-03, 2.084211725904822e-02,  -2.814992265313088e-02,
    ...
    for (sctl::Long t = 0; t < Ntrg_; t += VecLen) {
        ...
        for (sctl::Long s = 0; s < Nsrc; s++) {
            ...
            // evaluate the PSWF local kernel
            Vec xtmp = FMA(R2, rsc_vec, cen_vec);
            Vec rtmp = sctl::EvalPolynomial(xtmp.get(), coefs);
...
void st3d_near_kernel_directcp_vec_cpp_helper(const int32_t *nd, const Real *rsc, const Real *cen, const Real *bsizeinv,
    ...
    constexpr auto cdata = []() {
        if constexpr (digits <= 3) {
            return std::make_pair(
                std::array<Real, 8>{-1.200465301327137e-02, 4.400592204092247e-02, -7.288597161347318e-02,
                                    6.078910968844339e-02, -3.108708030232192e-02, 1.003402339570593e-02,
                                    5.363405306153075e-03, -5.553171339387891e-03},
                std::array<Real, 11>{2.250430121704944e-02, -9.787611559588733e-02, 2.070217916734389e-01,
            ...
            std::make_pair(std::array<Real, 16>{-6.778413276129699e-04, 4.066402381071667e-03, -1.124864783439655e-02,
                                                1.920489141603694e-02, -2.281183769709533e-02, 1.999165544932320e-02,
                                                -1.322112536620177e-02, 6.698004916394447e-03, -2.867817976329824e-03,
                                                8.503272900193856e-04, 6.879027750537969e-04, -6.923660302115852e-04,
                                                -4.270882823565761e-04, 3.190690821977045e-04, 3.599672200216620e-04,
                                                -2.326863819796524e-04},
                           std::array<Real, 18>{1.184705266078750e-03, -7.928021809420677e-03, 2.512862663913759e-02,
...
void st2d_near_kernel_directcp_vec_cpp_helper(const int32_t *nd, const Real *rsc, const Real *cen, const Real *bsizeinv,
    ...
    constexpr auto cdata = []() {
        if constexpr (digits <= 3) {
            return std::make_pair(
                std::array<Real, 8>{-3.998414794184642e-03, 1.324285664942096e-02, -1.893609240136285e-02,
                                    1.988081279390220e-02, -1.565735340857958e-02, 6.009474290405808e-03,
                                    1.461928833976798e-03, -1.654777943685357e-03},
                std::array<Real, 10>{6.825370013492135e-03, -2.738568594707985e-02, 4.963206753307496e-02,
            ...
            return std::make_pair(
                std::array<Real, 15>{-1.456047650277214e-04, 9.079956726538666e-04, -2.623397818356276e-03,
                ...
                std::array<Real, 18>{2.429808372877533e-04, -1.671550929931438e-03, 5.450474945249396e-03,
