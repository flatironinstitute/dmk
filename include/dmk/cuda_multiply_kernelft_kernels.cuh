...
// ===== launchers =====
template <typename Real>
inline void launch_multiply_cd2p_3d(const MultiplyCd2pArgs<Real> &args, cudaStream_t stream) {
...
template <typename Real>
inline void launch_multiply_stokeslet_3d_3d(const MultiplyStokeslet3DArgs<Real> &args, cudaStream_t stream) {
    if (args.n_boxes_at_level == 0)
        return;
    constexpr int block_size = 128;
...
template <typename Real>
inline void launch_multiply_stresslet_3d_3d(const MultiplyStresslet3DArgs<Real> &args, cudaStream_t stream) {
    if (args.n_boxes_at_level == 0)
        return;
    constexpr int block_size = 128;
...
#endif // DMK_CUDA_MULTIPLY_KERNELFT_KERNELS_CUH
