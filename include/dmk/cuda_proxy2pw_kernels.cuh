#ifndef DMK_CUDA_PROXY2PW_KERNELS_CUH
#define DMK_CUDA_PROXY2PW_KERNELS_CUH
...
__global__ void Proxy2PwMultiLevelKernel(
    ...
    int n_args)
{
    ...
    Proxy2PwArgs<Real> a = args[arg_idx];

    if (box_idx >= a.n_boxes_at_level)
        return;

    extern __shared__ unsigned char shared_raw[];
    Real *ff_slab = reinterpret_cast<Real *>(shared_raw);                      // 2 * n_order^2 reals
    Real *ff2_slab = ff_slab + 2 * (long)a.n_order * a.n_order;                // 2 * n_order * n_pw reals
    ...
    const long dst_off_complex =
        a.dst_offsets ? a.dst_offsets[box] : (long)box_idx * a.dst_stride_complex;
    ...
    for (int d = 0; d < a.n_charge_dim; ++d) {
        ...
        for (int m3 = 0; m3 < n_pw2; ++m3) {
            ...
            // proxy is real; poly2pw is complex; ff is complex.
...
template <typename Real>
inline void launch_proxy2pw_3d(const Proxy2PwArgs<Real> &args, cudaStream_t stream) {
    if (args.n_boxes_at_level == 0)
        return;

    constexpr int block_size = 128;

    const std::size_t shared_bytes = sizeof(Real) * 2 * (std::size_t)(args.n_order * args.n_order + args.n_order * args.n_pw);
...
template <typename Real>
inline void launch_proxy2pw_multilevel_3d(const Proxy2PwArgs<Real> *d_args,
                                          int n_args,
                                          int max_boxes,
                                          int n_order,
                                          int n_pw,
    ...
    const std::size_t shared_bytes = sizeof(Real) * 2 * ((std::size_t)n_order * n_order + (std::size_t)n_order * n_pw);
    ...
    if (err != cudaSuccess) {
        throw std::runtime_error(
            std::string("launch_proxy2pw_multilevel_3d: ") +
...
#endif // DMK_CUDA_PROXY2PW_KERNELS_CUH
