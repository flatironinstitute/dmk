...
__device__ inline void PwToProxyByBoxBody(const PwToProxyArgs<Real> &a,
                                          int box_idx,
                                          Real *ff_slab,
                                          Real *ff2_slab) {
    ...
    Real *proxy_dst = a.proxy_flat + proxy_off;
    ...
    for (int d = 0; d < a.n_charge_dim; ++d) {
        ...
        for (int k1 = 0; k1 < n_order; ++k1) {
            ...
            // Phase 1:
            // ff[m2, m3] = sum_m1 pw2poly[m1, k1] * pw_in[m1, m2, m3, d]
            ...
            // Phase 2:
            // ff2[k2, m3] = halve(m3) * sum_m2 pw2poly[m2, k2] * ff[m2, m3]
            ...
            // Phase 3:
            // proxy[k1, k2, k3, d] +=
            //   2 * Re(sum_m3 ff2[k2, m3] * pw2poly[m3, k3])
...
__global__ void PwToProxyByBoxKernel(PwToProxyArgs<Real> a) {
    ...
    Real *ff_slab =
        reinterpret_cast<Real *>(shared_raw);

    Real *ff2_slab =
        ff_slab + 2 * (long)a.n_pw * a.n_pw2;

    const int box_idx = blockIdx.x;

    PwToProxyByBoxBody<Real>(a, box_idx, ff_slab, ff2_slab);
}

template <typename Real>
__global__ void PwToProxyMultiLevelKernel(const PwToProxyArgs<Real> *args,
                                          int n_args) {
    const int box_idx = blockIdx.x;
...
template <typename Real>
inline void launch_pw_to_proxy_3d(const PwToProxyArgs<Real> &args,
                                  cudaStream_t stream) {
    ...
    const std::size_t shared_bytes =
        sizeof(Real) * 2 *
        (std::size_t)(args.n_pw * args.n_pw2 +
                      args.n_order * args.n_pw2);

    PwToProxyByBoxKernel<Real>
    ...
    if (err != cudaSuccess) {
        throw std::runtime_error(
            std::string("launch_pw_to_proxy_3d: ") +
            cudaGetErrorString(err));
    }
}

template <typename Real>
inline void launch_pw_to_proxy_multilevel_3d(const PwToProxyArgs<Real> *d_args,
                                             int n_args,
                                             int max_boxes,
                                             int max_n_pw,
                                             int max_n_pw2,
    ...
    const std::size_t shared_bytes =
        sizeof(Real) * 2 *
        ((std::size_t)max_n_pw * max_n_pw2 +
         (std::size_t)max_n_order * max_n_pw2);

    dim3 grid(max_boxes, n_args, 1);

    PwToProxyMultiLevelKernel<Real>
    ...
    if (err != cudaSuccess) {
        throw std::runtime_error(
            std::string("launch_pw_to_proxy_multilevel_3d: ") +
            cudaGetErrorString(err));
    }
}
