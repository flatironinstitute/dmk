c     This file contains the direct evaluation routines for the 2D 
c     Yukawa kernel K_0(kr)
c      
c     y2ddirectcp: direct calculation of potential for a collection
...
c
C***********************************************************************
      subroutine y2ddirectcp(ndim,nd,rlambda,sources,charge,ns,
     1    ztarg,nt,pot,thresh)
c**********************************************************************
c
...
c     quantities.
c
c     pot(x) = pot(x) + sum  q_{j} K_0(lambda |x-x_j|)
c                        j
c                 
...
c
      real *8 zdiff(ndim),dd,d,ztmp,threshsq,r,rkval
      real *8 besk0
      external besk0
      integer i,j,idim,k
...
c          dd = zdiff(1)**2 + zdiff(2)**2 + zdiff(3)**2
            ...
            r = sqrt(dd)
            rkval = besk0(rlambda*r)
            do idim=1,nd
               pot(idim,i) = pot(idim,i) + charge(idim,j)*rkval
            enddo
 1000       continue
         enddo
      enddo
