...
c     last modified on 04/03/2024 by Shidong Jiang
c
...
c     residual kernel poly expansion coefficients
      ...
      integer ilev
      integer i,j,k,iperiod0,ifself,ipoly
...
c     calculate memory needed for multipole and local planewave expansions
     ...
     1    nlevels,ifpwexpform,ifpwexpeval,lmptot,npw)
      if(ifprint .eq. 1)
...
c     calculate and allocate memory for tensor product grid for all levels
     ...
     1    nlevels,ifpwexpform,ifpwexpeval,lcoefsptot,norder)
...
c     windowed kernel at level 0
     ...
     1    beta,dim,bsize,eps,hpw(-1),npw,ws(-1),rl(-1))
...
c     difference kernels at levels 0, ..., nlevels-1
     ...
     1       beta,dim,bsize,eps,hpw(ilev),npw,ws(ilev))
...
c
     ...
     1    rmlexp(iaddr(1,ibox)),pswfft(1,-1))
...
c              form the pw expansion
     ...
     3             rmlexp(iaddr(1,ibox)))
               ...
               call dmk_multiply_kernelFT(nd,nexp,
     1              rmlexp(iaddr(1,ibox)),pswfft(1,ilev))
...
c                 eval at sources
     ...
     5                   ifdipole,rnormalsort,dipstrsort,
...
c------------------------------------------------------------------    
     ...
     1    nlevels,ifpwexpform,ifpwexpeval,lmptot,norder)
c     This subroutine determines the size of the array
c     to be allocated for multipole/local expansions
c
c     Input arguments
